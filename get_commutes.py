...
def get_strava_data(start_date_epoch, end_date_epoch):
    ...
    page = 1
    while True:
        query_string = {"after": start_date_epoch, "per_page": PER_PAGE, "page": page,
    ...
    for ride in strava_data:
        if ride['type'] == 'Ride':
            if ride['device_watts']:
                print("datum: " + ride['start_date'] + ", watts avg",
                      ride['average_watts'], "afstand:", ride["distance"]/1000,
                      "watts :", ride['weighted_average_watts'], "speed avg:",
                      '%.2f' % ((ride['distance'] / ride['moving_time']) * 3.6),
